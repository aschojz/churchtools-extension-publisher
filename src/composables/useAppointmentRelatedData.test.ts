...
describe('useAppointmentRelatedData', () => {
    ...
    it('only loads a linked source after the user requests it', async () => {
        ...
        expect(churchtoolsClient.get).toHaveBeenCalledWith(expect.stringContaining('include=eventServices'));
        expect(churchtoolsClient.get).toHaveBeenCalledWith(expect.stringContaining('limit=99'));
