...
const mountPanel = () => {
    ...
    return { store, wrapper: mount(PublisherAppointmentPanel, {
        props: panelProps,
...
describe('PublisherAppointmentPanel', () => {
    ...
    it('closes only after confirming an appointment', async () => {
        ...
        await wrapper.findAll('.design-dialog__footer button').at(-1)!.trigger('click');

        expect(store.selectedAppointmentKey).toBe('1:start');
        expect(wrapper.emitted('close')).toHaveLength(1);
    });

    it('synchronizes the shared dialog with the store', async () => {
        const { store, wrapper } = mountPanel();
        await nextTick();
