...
describe('image focus', () => {
    ...
    it('clamps focus values and rejects invalid dimensions', () => {
        expect(clampImageFocus({ x: -10, y: 120, zoom: 500 })).toEqual({ x: 0, y: 100, zoom: 300 });
        expect(
            calculateCoverCrop(
                { width: 0, height: 100 },
                { width: 50, height: 50 },
    ...
    it('zooms into the cover crop around the focus point', () => {
        expect(
            calculateCoverCrop(
                { width: 2400, height: 1200 },
                { width: 1000, height: 1000 },
        ...
        ).toEqual({ x: 450, y: 450, width: 600, height: 600 });
    });
});
