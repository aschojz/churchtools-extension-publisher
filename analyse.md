...
Release-Ergänzung vom 13. September 2026: Die erste öffentliche Vorschau ist als `v0.1.0` versioniert. `CHANGELOG.md` und README beschreiben Installation, bekannte Einschränkungen und den manuellen Releaseprozess. `npm run release` erstellt ein ZIP für `publisher-26`, ohne lokale Anmeldedaten, Instanz-URL oder E2E-Modus zu übernehmen. Wiederholte Paketierung ersetzt das Archiv vollständig und wird durch einen Regressionstest abgesichert. Die lokalen Monorepo-Abhängigkeiten sind entfernt: URL-Parameter und Gruppenstatus-IDs liegen als getestete Hilfsfunktionen im Projekt, der vorhandene API-Typ-Snapshot wurde unverändert aus der generierten Quelle aktualisiert. Installation, Typecheck und Build sind damit unabhängig von einem benachbarten ChurchTools-Checkout. Die unten dokumentierten Produktrisiken bleiben bestehen und werden durch die Veröffentlichung nicht als behoben eingestuft.

Verifikation der Release-Vorbereitung: Eine frische Installation in einem separaten Verzeichnis ohne ChurchTools-Checkout besteht Typecheck, 291 Tests in 62 Testdateien, zehn Chromium-Browsertests und den Produktionsbuild. Die hohe `nanoid`-Warnung wurde durch ein kompatibles Patchupdate auf 3.3.19 behoben; fünf mittlere Warnungen im Abhängigkeitsbaum von `node-vibrant` bleiben offen. Die älteren Messungen am Ende dieses Audits beziehen sich weiterhin auf den damaligen Stand.
